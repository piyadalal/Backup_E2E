...
class BareBones:
    ...
    def After_Reset(self):
        scan_start_screen = "C:\Users\PRDA5207\Github_Repos\sky-onbox-e2e-skyq-pa-automation\Storm_test_automation\Ref_images\start_scan.bmp"
        #ti.sleep(120)
        #blue screen appears to press home
        self.__api.PressButton('Home')
        ti.sleep(1)
        self.__api.PressButton('Home')
        ti.sleep(1)
        #self.is_scanStartScreen(scan_start_screen)
        from Start_Scan import Start_Scan
        st = Start_Scan(self.__configServer, self.__serverIP, self.__slotNo,None)
        if self.__slotNo == 11:
            st.startScan_SatCR()
        elif self.__slotNo == 10:
            st.startScan_Dual()
        else:
            st.startScan_Single()
